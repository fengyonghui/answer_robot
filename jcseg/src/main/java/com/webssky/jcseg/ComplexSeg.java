...
public class ComplexSeg extends ASegment {
    ...
    public ComplexSeg() throws IOException {
        super();
    }

    public ComplexSeg(ADictionary dictionary){
        super(dictionary);
    }

    public ComplexSeg(Reader input) throws IOException {
        super(input);
    }

    public ComplexSeg(Reader input, ADictionary dic) throws IOException {
        super(input, dic);
    }
                ...
                for (int y = 0; y < mword2.length; y++) {
                    ...
                    if (idx_3 < chars.length) {
                        ...
                        for (int z = 0; z < mword3.length; z++) {
                            ...
                            wArr.add(mwords[x]);
                            wArr.add(mword2[y]);
                            if (mword3[z] != null && mword3[z].getType() != ILexicon.UNMATCH_CJK_WORD)
                                wArr.add(mword3[z]);
